...
export const getRateComparedUSD = to =>
  fetchData({ path: "live", params: { currencies: to } }).then(
    json => json.quotes[`USD${to}`]
  );
...
export const getRate = async (from, to) => {
  ...
  if (to === "USD") {
    return getRateComparedUSD(from).then(rate => 1 / rate);
  }
  ...
  //convert to USD
  await getRateComparedUSD(from).then(rate => (fromRateComparedUSD = 1 / rate));
  //USD to new currency
  await getRateComparedUSD(to).then(rate => (toRateComparedUSD = rate));
